...
/*La classe math_sessio conté la informació necessària per desenvolupar una sessió
del programa ESINMATH. Guarda en tot moment l’estat de la sessió i executa comandes
que consulten i/o modifiquen aquest estat.
Implementació: La representació d’aquesta classe estarà en el fitxer math_sessio.rep
i la implementació en el fitxer math_sessio.cpp. L’ús de la classe list de l’STL és
exclusivament per l’entrada i sortida de dades de diversos mètodes de la classe i no pot
ser usada amb una altra finalitat.
*/
class math_sessio
{
public:
